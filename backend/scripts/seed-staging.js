 ...
 *
 * Creates operators and properties for staging environment
...
const STAGING_URL = "https://api.ta-da.co/";
const OPERATORS_COUNT = 5;
const PROPERTIES_PER_OPERATOR = 4;

// Operator data for staging
const operators = [
  {
    email: "staging-operator1@tada.com",
  ...
  {
    email: "staging-operator2@tada.com",
    password: "StagingPass123!",
    name: "Sarah Johnson",
  ...
  {
    email: "staging-operator3@tada.com",
    password: "StagingPass123!",
    name: "Michael Brown",
  ...
  {
    email: "staging-operator4@tada.com",
    password: "StagingPass123!",
    name: "Emma Wilson",
  ...
  {
    email: "staging-operator5@tada.com",
    password: "StagingPass123!",
    name: "David Taylor",
...
// Property data templates (same as local)
...
async function registerOperator(operatorData) {
  try {
    ...
    const response = await axios.post(`${STAGING_URL}/auth/register`, {
    ...
    if (error.response?.status === 409) {
      ...
      try {
        const loginResponse = await axios.post(`${STAGING_URL}/auth/login`, {
          email: operatorData.email,
          password: operatorData.password,
        });
...
async function addPropertyImages(token, propertyId, images) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      await axios.post(
        `${STAGING_URL}/properties/${propertyId}/media`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
async function seedStagingDatabase() {
  ...
  try {
    await axios.get(`${STAGING_URL}/health`);
    console.log("✅ Staging server is running");
  } catch (error) {
    console.error("❌ Staging server is not accessible");
    console.log("Please check if staging environment is running");
    process.exit(1);
  }
  ...
  console.log("\n🎉 Staging database seeding completed!");
  console.log("=".repeat(50));
  console.log(`📊 Summary:`);
  ...
  console.log("");
  console.log("📝 STAGING LOGIN CREDENTIALS:");
  console.log("=============================");
  operators.forEach((operator, index) => {
    console.log(`${index + 1}. ${operator.email} / ${operator.password}`);
  });
  console.log("");
...
if (require.main === module) {
  seedStagingDatabase().catch((error) => {
    console.error("❌ Staging seeding failed:", error.message);
    process.exit(1);
  });
}

module.exports = { seedStagingDatabase };
