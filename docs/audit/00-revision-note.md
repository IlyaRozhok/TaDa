...
## `docker-compose.yml` — Redis убран осознанно, правка доведена до рабочего вида
...
2. **Инфраструктура:** доступ к stage и prod (VPS Hetzner) есть. Фактические
   ...
   истины — `05`, Фаза 3.1.
3. **Redis не нужен.** Удалён из кода на `develop`; **сервис, том и `depends_on` убраны
   из `docker-compose.yml` — это осознанное решение, откат не требуется.**
   Обоснование: в аутентификации Redis не участвует (cookie + JWT с `refresh_token_hash`
   в БД), кэша на нём нет. Остаточная чистка — только `REDIS_*` в env и артефакты на хостах.
