...
**Текущий шаг: 0.2** — 🟡 в работе, ветка `fix/remove-test-sentry`, PR открыт, не мержен.
Предыдущий 0.1 влит в `develop` (PR #48), ждёт проверки на stage.
Следующие: 0.4 (путь миграций — нужно решение по `migrationsRun`), затем 0.3 (сверка миграций на хостах, read-only).
...
| 0.2 | Убрать `GET /api/test-sentry` | 🟢 | 🟡 | `fix/remove-test-sentry` | ⬜ | 2026-07-28 | R17. Эндпоинт удалён, других ссылок в коде нет. PR открыт, **не мержен** |
