...
**Current step: 4.3 part 3** — finish the remaining 37 `strict` errors, then turn the flag on.

**Phases 2, 2A, 3 (the parts we took on) and 4 are merged.** Everything from PR #61 to #75 is in `develop`. Still genuinely open: 3.1 and 3.2 (deferred by the owner), 3.3b (frontend `console.*`/Sentry), 4.3 part 3, and phases 5–7.
...
**Phase 2 closed.** Phase 2A closed too: 2А.1 skipped by decision, 2А.2 (#63), 2А.3 (#64).
