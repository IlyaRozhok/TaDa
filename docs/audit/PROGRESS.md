...
**Current step: 4.3 part 3** — 22 of the 30 `strict` blockers cleared; **the flag is still off**, blocked on two modelling questions for the owner (see the note below).
...
| 4.3 **part 3** | Finish the remaining errors and switch `strict` on | 🟡 | 🟡 | PR open | ✅ | 2026-08-01 | Turning `strict` on reported **30**, not the 37 measured earlier — seven went with the files 5.3a deleted. **22 fixed**, `strict` left **off** because 8 remain and a branch that does not type-check cannot be merged. The fixes are real types, not silencers: a shared `PreferencesFieldValue` replaces `unknown` in the nine wizard step components and in `updateField`; `BuildingFormData.pets` admits the `null` the code assigns; `metro_stations` reads get a base; `filter(Boolean)` becomes a type guard; the matches page names its category shape. **No `as any`, no `!`, no `@ts-ignore`.** The residue is two questions, not eight bugs — see «Noted along the way» |
