...
**Текущий шаг: 0.4** — 🟡 в работе, ветка `fix/migration-run-config`, PR открыт, не мержен.
Закрыто: 0.1 (PR #48), 0.2 (PR #50), 0.3 (сверка на хостах — pending = 0 на prod и stage).
После 0.4 **Фаза 0 закрыта** → следующая Фаза 1, начиная с 1.1.
...
|---|---|---|---|---|---|---|---|
| 0.1 | Закрыть эскалацию привилегий: `PUT /users/:id/role` → `@Auth("admin")`, убрать самопроверку `req.user.id !== id`, валидация роли | 🟡 | 🟡 | #48 merged | ⬜ | 2026-07-28 | R1. Влит в `develop`. **Ждёт проверки на stage** → после неё Stage=✅ и статус ✅ |
| 0.2 | Убрать `GET /api/test-sentry` | 🟢 | 🟡 | #50 merged | ⬜ | 2026-07-28 | R17. Влит в `develop`. Эндпоинт удалён, других ссылок в коде нет. **Ждёт проверки на stage** → после неё Stage=✅ и статус ✅ |
...
| 0.4 | `migrationsRun: false` + починить путь миграций в `typeorm.config.ts` | 🟡 | 🟡 | `fix/migration-run-config` | ⬜ | 2026-07-29 | R2. Разблокирован 0.3. Путь через `join(__dirname, …)` → `dist/database/migrations`. Применение миграций остаётся явным шагом `mig:run:prod` в CI. PR открыт, **не мержен** |
