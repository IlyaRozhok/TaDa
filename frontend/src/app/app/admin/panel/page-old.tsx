    ...
    try {
      ...
      switch (activeSection) {
        ...
        case "properties":
          ...
          break;
        case "preferences":
          endpoint = `${apiUrl}/preferences/${selectedItem.id}`;
          break;
      }
              ...
              {properties.map((property) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-full flex items-center justify-center shadow-sm">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 border border-blue-200 capitalize">
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {preferences.map((pref) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                  {pref.min_price && pref.max_price ? (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium">
                        £{pref.min_price.toLocaleString()} - £
                        {pref.max_price.toLocaleString()}
                      </span>
                    </div>
                  ) : (
                  ...
                  {pref.min_bedrooms ? (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium">
                        {pref.min_bedrooms}
                        {pref.max_bedrooms &&
                        pref.max_bedrooms !== pref.min_bedrooms
                          ? `-${pref.max_bedrooms}`
                          : "+"}
                      </span>
                    </div>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {pref.property_type ? (
                    <span className="inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800 border border-green-200 capitalize">
                      {pref.property_type}
                    </span>
                  ) : (
...
const AddEditUserModal = () => {
  if (!showModal || (showModal !== "add" && showModal !== "edit"))
    return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isEditing) {
        body.password = formData.password || "defaultPassword123";
      }
            ...
            {!isEditing && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/80 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-slate-400 transition-colors shadow-sm"
...
// Modal Components for Properties
const AddEditPropertyModal = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || `Failed to ${isEditing ? "update" : "create"} property`
      );
    } finally {
      setIsActionLoading(false);
    }
...
// Modal Components for Preferences
const AddEditPreferencesModal = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("❌ Preferences operation failed:", err);
      setError(
        err.message ||
          `Failed to ${isEditing ? "update" : "create"} preferences`
      );
